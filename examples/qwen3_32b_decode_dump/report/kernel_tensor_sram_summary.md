...
- All `pl.slice` of GM tensors use fixed, 512-B-aligned storage shapes.
...
```python
k_tile = pl.slice(k_cache, [SEQ_TILE, HEAD_DIM], [row0, 0],
                 valid_shape=[valid_len, HEAD_DIM])
v_tile = pl.slice(v_cache, [SEQ_TILE, HEAD_DIM], [row0, 0],
                 valid_shape=[valid_len, HEAD_DIM])
```
